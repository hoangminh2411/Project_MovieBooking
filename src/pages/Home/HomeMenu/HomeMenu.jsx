...
function HomeMenu(props) {
    ...
    const renderCumRap = () => {
        return heThongRapChieu?.map((heThongRap, index) => {
            return <TabPane 
                        style={{ maxHeight: 600, minHeight: 600, overflow: 'auto' }} 
                        key={index} 
                        tab={
                                <div className="border-b-2 pb-1">
                                    <img src={heThongRap.logo} className={keyActiveBrand == index ? 'rounded-full w-10 h-10 opacity-100' : 'rounded-full w-10 h-10 opacity-20'} alt="" />
                                </div>
                            }>
                    <Tabs 
                        defaultActiveKey={0} 
