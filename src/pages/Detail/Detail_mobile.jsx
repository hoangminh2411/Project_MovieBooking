...
import { Tabs, Select, Progress } from 'antd'
...
export default function Detail_mobile(props) {
  ...
  return (<>
    ...
    <div className="w-screen h-screen overflow-auto" style={{ backgroundColor: 'rgb(10, 32, 41)' }}>

      <div className="relative">

        <img className="rounded-3xl w-full h-full" src={filmDetail.hinhAnh} alt="" />
        <div className="detailContentMobile bg-white shadow-xl pb-5 pt-10 px-5 absolute bottom-0  rounded-3xl  w-full h-auto" style={{ bottom: '0%', zIndex: '1', height: '50%' }}>
          <div className="" >
          ...
          <div className="flex justify-between mt-14">
            {filmDetail?.dangChieu?<HashLink to="#dateSelect">
            ...
            </HashLink>:<h3 className="text-red-500 text-xl font-bold">COMING SOON.....</h3>}
          ...
          <div onClick={() => {
            ...
            handleShowTrailer()
          }} className="hover:opacity-50 cursor-pointer bg-white w-20 h-20 flex justify-center items-center absolute  rounded-full shadow-xl   top-0 left-1/2" style={{ transform: 'translate(-50%,-50%)' }}><CaretRightOutlined style={{ fontSize: '40px', zIndex: '11000' }} /></div>
      ...
      {filmDetail?.dangChieu?<div className="text-white " style={{ backgroundColor: 'rgb(10, 32, 41)', paddingTop: '10%', height: '700px' }}>
          ...
          <Tabs className="" tabPosition="top" centered>
            {filmDetail.heThongRapChieu?.map((heThongRap, index) => {
              return <TabPane key={index} tab={<div className="flex flex-col items-center justify-center">
                ...
                <div className="mb-5">
                  ...
                  {heThongRap.cumRapChieu?.map((cumRap, index) => {
                    if (cumRap.tenCumRap === rap) {
                      return <p key={index} style={{ paddingLeft: '11px' }} className="text-sm text-gray-500">{cumRap.diaChi}</p>
                    }
                    return ''
                  })}
                </div>
                  ...
                  {heThongRap.cumRapChieu?.map((cumRap, index) => {
                    if (cumRap.tenCumRap === rap) {
                      return cumRap.lichChieuPhim?.map((film, index) => {
                        return <NavLink key={index} to={`/checkout/${film.maLichChieu}`}>
