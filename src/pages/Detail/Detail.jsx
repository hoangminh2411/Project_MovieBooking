...
export default function Detail(props) {
  ...
  return (
    <div >
      ...
      <div style={{background:'linear-gradient(to top, rgb(10, 32, 41) 10%, transparent 70%)'}} className="absolute bottom-0 left-0 h-screen w-full -z-0 "></div>
        ...
        <div className=" flex justify-between items-center xl:px-40 lg:px-20   ">
          <div className="flex">
            ...
            <div className="text-white ml-3 w-2/3 flex flex-col justify-center">
              ...
              <a href="#thongTinPhim" className="rounded-lg w-56     bg-red-700 text-base opacity-90 hover:opacity-100 hover:text-white cursor-pointer text-white px-6 py-3 text-center   ">Thông tin phim</a>
      ...
      <div className="py-5" style={{ backgroundColor: `rgb(10, 32, 41)`, minHeight: '500px' }}>
        ...
        <div className="container mx-auto w-full text-white" style={{ maxWidth: '1024px' }}>
          <Tabs onTabClick={handleAccessTab} defaultActiveKey="1" centered>
            <TabPane tab={<h1 id="thongTinPhim" className={`tabNormal ${keyActiveTab === '1' ? 'tabActive' : ''}`}>THÔNG TIN</h1>} key="1">
            ...
            <TabPane tab={<h1 className={`tabNormal ${keyActiveTab === '2' ? 'tabActive' : ''}`}>ĐÁNH GIÁ</h1>} key="2">
                <DanhGia maPhim = {props.match.params}/>
            </TabPane>
            {filmDetail.dangChieu===true ?<TabPane tab={<h1 className={`tabNormal ${keyActiveTab === '3' ? 'tabActive' : ''}`}>LỊCH CHIẾU</h1>} key="3" style={{ minHeight: 300 }}>
