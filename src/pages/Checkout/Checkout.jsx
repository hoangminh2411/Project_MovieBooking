...
export default function Checkout(props) {
  ...
  let soLuongGheDangDat = danhSachGheDangDat?.length
  useEffect(() => {
  ...
  const renderGheDangChon = () => {
    return danhSachGheDangDat.map((gheDD, index) => {
      return <span key={index} className="text-xl text-green-800 ml-1">{`${toLetters(Math.floor((gheDD.stt - 1) / 16) + 1)}${(gheDD.stt) - 16 * Math.floor((gheDD.stt - 1) / 16)} `}{soLuongGheDangDat <= 1 || index === soLuongGheDangDat - 1 ? '' : ','}</span>
    })
  }
  const renderTienGhe = () => {
  ...
  return (
    <>
      ...
      <Modal
        visible={isModalVisible && soGheChoPhep}
        closable={false}
        centered={true}
        footer={<Button key="back" onClick={handleOk}
        ...
        <div className="flex flex-col items-center">
          <div className="flex justify-center">
            <img className="h-24 " src={TICKET} alt="ticket" />
            <img className="h-24 " src={TICKET} alt="ticket" />
            <img className="h-24 " src={TICKET} alt="ticket" />
          </div>
          <p className="text-center font-bold text-2xl  text-gray-500">
            {`Bạn không được mua quá ${soGheKhongVuotQua} vé!`}
          </p>
        </div>
      </Modal>
      <div className="grid grid-cols-12">
        <div className="col-span-12 xl:col-span-9">
          ...
          <div className="w-full h-28 border shadow-md   flex flex-row items-center pr-5 xl:pr-0 justify-around ">
            ...
            <div className="xl:hidden h-full flex flex-row basis-full items-center justify-center">
              <p className={`${soLuongGheDangDat > 0 ? 'hidden' : 'block'} text-gray-500 font-bold text-2xl mb-0`}>1.CHỌN GHẾ</p>
              <p className={`${soLuongGheDangDat > 0 ? 'block' : 'hidden'} text-gray-500 font-bold text-2xl mb-0`}>2.ĐẶT VÉ</p>
            </div>
            <div className="hidden xl:h-full xl:flex xl:flex-row xl:items-center xl:pl-20  xl:basis-4/5">
              <Steps>
                <Step
                  className={`transition-all duration-300 ${soLuongGheDangDat > 0 ? 'opacity-100' : 'opacity-20'}`}
                  status={soLuongGheDangDat > 0 ? 'finish' : 'process'}
                  title="Chọn ghế"
                  icon={<UserOutlined />}
                />
                <Step
            ...
            <NavLink
              to="/profile"
              className="xl:h-full xl:flex xl:flex-col xl:justify-center xl:items-center xl:basis-1/5 ">
              <img
                className="inline-block rounded-full xl:w-14 xl:h-14 w-24"
                src={`https://i.pravatar.cc/150?u=${userLogin.hoTen}`}
                alt="user avatar"
              />
              <p className="hidden xl:block xl:mb-0 xl:text-center xl:font-medium xl:text-xl xl:text-gray-300">
                {userLogin.hoTen}
              </p>
            </NavLink>
          </div>
          ...
          <div
            className="relative flex flex-col justify-start items-center mt-5"
        ...
        <div className="xl:col-span-3">
          <div
            className="hidden xl:block h-screen w-full border shadow-xl  py-0 relative overflow-auto"
            style={{ padding: '0 8%' }}>
        ...
        <div className="absolute cursor-pointer  w-full left-0 bottom-0" >
          <div
            className="xl:hidden w-1/2 left-0 -bottom-1 fixed py-3 border-t bg-white text-2xl border-gray-300 font-bold"
            style={{ height: '56px' }}
          >
            {renderGheDangChon()}
          </div>
          <button
            onClick={handleDatVe}
            disabled={soLuongGheDangDat <= 0}
            className={`transition-all duration-300 fixed border-gray-300 border-t  right-0 -bottom-1 w-1/2 xl:w-1/4 ${soLuongGheDangDat > 0 ? 'hover:bg-orange-500 bg-orange-400' : 'bg-gray-500'}  text-white text-center py-3 font-bold text-2xl`}
          >
            Đặt vé
          </button>
        </div>
